    ...
    <div id="content" class="content">
        ...
        <div class="main">
            ...
            if ($_COOKIE['loged'] == ''):
                ...
                echo '<p style="color: #2C2C2C; padding-top: 20px"><a style="color: #2C2C2C" href="/php/auth.php">Авторизуйтесь</a>, чтобы видеть свою корзину</p>';
            else:

                // $basketcheckfor = mysqli_query($connect, "SELECT * FROM `basket` WHERE `user_id` = '$userid'");
                // $basketche = mysqli_fetch_assoc($basketcheckfor);
                $basketinfo = mysqli_query($connect, "SELECT basket.id as id, icon, name, price FROM `basket` JOIN `programs` ON `itemid` = `programs`.`id` WHERE `userid` = '$userid'");

                $basketcheck = mysqli_fetch_assoc(mysqli_query($connect, "SELECT * FROM `basket` WHERE `userid` = $userid"));
                // $basketcheckfor = mysqli_fetch_all($basketcheckfor);
                ...
                if (!isset($basketcheck)) {
                    ...
                    while ($basket = $basketinfo->fetch_assoc()) {

                        echo '<div class="libitem">

                        <div class="libicon">
                            <img src="' . $basket["icon"] . '" alt="">
                        </div>
        
                        <div style="margin-top: 33px" class="iteminfo">
                            <h4>' . $basket["name"] . '</h4>
                            <p>' . $basket["price"] . ' RUB</p>
                        </div>

                        <form action="/code/delbasket.php" method="post">
                            <input name="basketid" type="hidden" value="' . $basket["id"] . '">
                            <button class="downloadbtn">Удалить</button>
                        </form>
        
                        </div>';

                        $basketprice = $basketprice + $basket["price"];

                    }


                    echo '<h3 style="padding-top: 20px">Сумма покупки: <span>' . $basketprice . ' RUB
                </span></h3>';
                ...
                ';
                    ...
                    <form action="/php/purchase.php" method="post">
                        <input name="sum" type="hidden" value="<?= $basketprice ?>">
                        <button class="oplata">Оплатить корзину</button>
                    </form>
